...
public class PunchCardDao {
	...
	public int findTodayPunchCard(PunchCard punchCard) {
		...
		List<StudentPlanSelection> list = jdbcTemplate.query(sql, PunchCardMapper, punchCard.getStudentNo(),punchCard.getPlanNo(),punchCard.getPunchCardTime());
	...
	public int punchCard(PunchCard punchCard) {
		...
		int flag = jdbcTemplate.update(sql,punchCard.getPlanNo(),punchCard.getStudentNo(),punchCard.getPunchCardTime(),punchCard.getTimeLength());
		if (flag > 0)
			return 1;
		return 0;
	}
